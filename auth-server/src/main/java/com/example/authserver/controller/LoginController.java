...
public class LoginController {
    ...
    public String register(@Valid UserRegisterVo vos, BindingResult result,
        ...
        //如果有错误回到注册页面
        if (result.hasErrors()) {
            Map<String, String> errors = result.getFieldErrors().stream().collect(Collectors.toMap(FieldError::getField, FieldError::getDefaultMessage,(k1,k2)->k1));
            attributes.addFlashAttribute("errors",errors);
            //效验出错回到注册页面  使用重定向方式避免页面重刷而导致重复发送register请求
